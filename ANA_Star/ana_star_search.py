...
class ana_star_planner(object):
	...
	# E(S) cost
	def maximal_cost_e(self, G, g_s, h_s):
		e_s = (G - g_s)/(h_s + 0.01)
		return e_s

	# Prune node
	def prune_node(self):
		# IF node doesn't contribute to improve_solution prune it
		new_queue = PriorityQueue(0)
		...
		while not self.open_nodes.empty():
			...
			h_s = self.heuristic_cost(current_node, self.goal_node)

			if g_s + h_s < self.G:
				e_s = self.maximal_cost_e(self.G, g_s, h_s)
				entry = (e_s, g_s, current_node )
				new_queue.put(entry)
		
		self.open_nodes = new_queue

	# Expands current node in an obstacle free space
	...
	def improve_solution(self):
		...
		while not self.open_nodes.empty():
			e_s, g_s, current_node = self.open_nodes.get()
			...
			for i in self.expand_nodes(current_node):
				...
				if i not in self.expanded or cost < self.expanded[i]:
					...
					if total_cost < self.G:
						e_next_node = self.maximal_cost_e(self.G, cost, h_cost )
						self.open_nodes.put((e_next_node, cost, i ))

					self.prev_node[i] = current_node

	# Logs data
	def log_data(self, run_time, iteration):
		self.performance_data_es.append(self.E)
		...
		print('E cost: ',self.performance_data_es[-1])

	# Main node
	def run(self):
		h_cost = self.heuristic_cost(self.start_node, self.goal_node)
		e_s = self.maximal_cost_e(self.G, 0, h_cost)
		current_node = self.start_node
		self.open_nodes.put( (e_s, 0 ,current_node ) )
...
def search(map):
	...
	I believe in you
		-Gunnar (the TA)-
	'''
	ana_search_algorithm = ana_star_planner(im, start, end)
	ana_search_algorithm.run()

	expanded.update(ana_search_algorithm.expanded)
	# visualize_search("out1.png") # see what your search has wrought (and maybe save your results)

	frontier.update(ana_search_algorithm.frontier)
	# visualize_search("out2.png") # see what your search has wrought (and maybe save your results)

	path.extend(ana_search_algorithm.path)
	visualize_search("out_anastar.png") # see what your search has wrought (and maybe save your results)
